...
import com.example.marsphotos.data.local.entities.CargaAcademicaEntity
...
fun CargaAcademicaScreen(
    ...
    Column(
        ...
        when (snUiState) {
            ...
            is SNUiState.CargaAcademicaSuccess -> {
                if (snUiState.carga.isEmpty()) {
                    Box(
                        modifier = Modifier.fillMaxSize(),
                        contentAlignment = Alignment.Center
                    ) {
                    ...
                    LazyColumn(
                        ...
                        items(snUiState.carga) { materia ->
                            CargaAcademicaCard(materia)
                        }
                    }
                }
            }
...
@Composable
private fun CargaAcademicaCard(materia: CargaAcademicaEntity) {
    Card(
        modifier = Modifier.fillMaxWidth()
    ) {
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            InfoRow("Grupo", materia.grupo)
            InfoRow("Créditos", materia.creditos.toString())
            InfoRow("Docente", materia.docente)
